...
TFile *f1 = TFile::Open("2018bkg/DY+Jets_ditau_2016.root");
f1->GetObject("h1",h[0]);

TFile *fstat = TFile::Open("2018bkg/DY+Jets_ditau_2016.root");
fstat->GetObject("h1",hstat);

TFile *f2 = TFile::Open("2018bkg/W+Jets_ditau_2016.root");
f2->GetObject("h1",h[1]);

TFile *f3 = TFile::Open("2018bkg/QCD_ditau_2016.root");
f3->GetObject("h1",h[2]);


TFile *f4 = TFile::Open("2018bkg/ST_tt_ditau_2016.root");
f4->GetObject("h1",h[3]);

TFile *f5 = TFile::Open("2018bkg/VV_total_ditau_2016.root");
f5->GetObject("h1",h[4]);
...
double fac = (0.01*xsec[i]*59700)/(100000);
hsignal[i]->Scale(fac);
}
...
c1->Modified();
        ...
        double y_legend = hs->GetMaximum() + 37;
        double x_legend = 170;
...
c1->SaveAs("bkg_ditau_2018.root");
c1->SaveAs("bkg_ditau_2018.pdf");
	}
