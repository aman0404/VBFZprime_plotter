...
TFile *f1 = TFile::Open("faketau_etau_all.root");
f1->GetObject("h1",h0[0]);

TFile *fstat = TFile::Open("faketau_etau_all.root");
fstat->GetObject("h1",h0stat);

TFile *f2 = TFile::Open("ttbar_etau_all.root");
f2->GetObject("h1",h0[1]);

TFile *f3 = TFile::Open("prompt_etau_all.root");
f3->GetObject("h1",h0[2]);

TFile *fdata = TFile::Open("allData.root");
//TFile *fdata = TFile::Open("20percent_data/allData.root");
fdata->GetObject("NDiJetCombinations/Electron1Tau1ReconstructableMass",hdata);
...
htotal_data->GetXaxis()->SetTitle("m(e,#tau_{h},E_{T}^{miss}) [GeV]");
...
//      TLatex *   tex = new TLatex(105,400,"CMS");
   	...
   	tex1->Draw();
        ...
        TLatex * tex2 = new TLatex(x_pos1,y_legend,"137.1 fb^{-1} (13 TeV)");
