...
TFile *f1 = TFile::Open("fakebkg_etau_allyear.root");
f1->GetObject("h1",h[0]);

TFile *fstat = TFile::Open("fakebkg_etau_allyear.root");
fstat->GetObject("h1",hstat);

TFile *f2 = TFile::Open("ttbar_etau_allyear.root");
f2->GetObject("h1",h[1]);

TFile *f3 = TFile::Open("prompt_etau_2016.root");
f3->GetObject("h1",h[2]);

TFile *f31 = TFile::Open("prompt_etau_2017.root");
f31->GetObject("hist",h[3]);

TFile *f32 = TFile::Open("prompt_etau_2018.root");
...
//hs->GetXaxis()->SetTitle("M_{#mu#tau_{h},MET} [GeV]");
